...
def space(distance):
    ...
    cv2.circle(blank_image, (700, 800), 100+distance, (255, 255, 255), -1)
    cv2.circle(blank_image, (700, 200), 100+distance, (255, 255, 255), -1)
...
# Finding the X and Y positions based on RPM.
    ...
    while queue:
        ...
        for action in actions:
            ...
            if new_position is not False:
                ...
                image[1000-int(new_position[1]*euc),int(new_position[0]*euc)]=0
...
def publish_actions(action_list):
    ...
    for action in action_list:
        ...
        # Finding linear velocity from wheel velocities
        lin_vel = (r_w/2)*(w_l + w_r)

        # Finding angular velocity from wheel velocities
        ang_vel = ((w_r - w_l)*(r_w))/(2*r_rob)

        # Normalizing values before input
        lin_vel = np.sign(lin_vel)*(lower_lin + (upper_lin - lower_lin) * abs(lin_vel))
        ang_vel = np.sign(ang_vel)*(lower_ang + (upper_ang - lower_ang) * abs(ang_vel))


        lin_vel = scale_lin*lin_vel
        ang_vel = scale_ang*ang_vel

        print("Lin, ang vel is {}, {}".format(lin_vel,ang_vel))

        vel_msg.angular.z = -1*scale_ang*ang_vel
        ...
        vel_msg.linear.x = scale_lin*lin_vel
...
# Finding the action, given two states.
def find_action(parent,current,distance,actions):

    i = 0
    for action in actions:
        i = i+1

        new_position, cost = check_move(action, parent, distance)

        if (new_position == current).all():
            return action
...
def find_final_goal(goal_nodes, initial_pos, backinfo, image, distance, actions):
    ...
    while not (parent[0]==0 and parent[1] == 0):
        ...
        action = find_action(parent, current, distance, actions)
...
if __name__ == '__main__':
    ...
    # Initializing parser for arguments
    ...
    # Initializing Rosnode for publishing
